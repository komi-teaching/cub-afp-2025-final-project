...
abbrev f_body : Expr := (.funCall "g" [ "x" + (.value (.nat 1)) ]) + "x"
...
lemma f_steps
  : SmallSteps (defs.insert "x" 0)
      (Expr.funCall "g" ["x" + (.value (.nat 1))] + Expr.var "x") (.value (.nat 1)) := by
  let env := defs.insert "x" 0
  calc
    SmallSteps env _ ((.value (.nat 1)) + "x") := by
      apply (SmallSteps.with_ctx (.binOpLhs .hole .add "x") rfl rfl)
      simp only [Ctx.updateEnv]
      calc
        SmallStep env _ (.funCall (.value (.closure ["x"] "x")) ["x" + (.value (.nat 1))]) := by
          step_auto_context
          apply HeadSmallStep.var_step
          simp [env, Std.HashMap.getElem_insert]
        SmallStep env _ (.letIn "x" ("x" + (.value (.nat 1))) "x") := by
          step_auto_context
          apply (HeadSmallStep.fun_step ?_ rfl)
          try rw [defs_g]
          simp
        SmallSteps env _ (.letIn "x" (.value (.nat 1)) "x") := by
          apply (SmallSteps.with_ctx (.letInExpr "x" .hole "x") rfl rfl)
          simp only [Ctx.updateEnv]
          calc
            SmallStep env _ ((.value (.nat 0)) + (.value (.nat 1))) := by
              step_auto_context
              apply HeadSmallStep.var_step
            ...
            SmallStep env _ _ := by
              ...
              constructor
        SmallStep env _ (.letIn "x" (.value (.nat 1)) (.value (.nat 1))) := by
          apply SmallStep.with_ctx (.letInBody "x" 1 .hole) (by simp; rfl) (by simp; rfl)
            (?_ : SmallStep _ "x" (.value (.nat 1)))
          step_auto_context
          apply HeadSmallStep.var_step
          rw [Std.HashMap.getElem?_insert_self]
          rfl
        SmallStep env _ (.value (.nat 1)) := by
          step_auto_context
          constructor
    SmallStep env _ ((.value (.nat 1)) + (.value (.nat 0))) := by
      step_auto_context
      constructor
      simp [env]
      rfl
...
example : SmallSteps defs (.funCall "f" [(.value (.nat 0))]) (.value (.nat 1)) := by
  calc
    SmallStep defs _ (.funCall (.value (.closure ["x"] f_body)) [(.value (.nat 0))]) := by
      apply SmallStep.ctx_step (.funCallBody .hole [(.value (.nat 0))]) (by simp) (by simp)
        (?_ : HeadSmallStep _ "f" (.value (.closure ["x"] f_body)))
      constructor
      simp
    SmallStep defs _ (.letIn "x" (.value (.nat 0)) f_body) := by
      apply SmallStep.hole_step
      apply (HeadSmallStep.fun_step ?_ rfl)
      try rw [defs_f]
      simp
    SmallSteps defs _ (.letIn "x" (.value (.nat 0)) (.value (.nat 1))) := by
